...
class Exp_Short_Term_Forecast(Exp_Basic):
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            self.model.train() #切换模型到训练模式（空函数没什么用）
            epoch_time = time.time() #记录起始时间
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                #size = batch_size,pred_len,feature_dim
                dec_inp = torch.zeros_like(batch_y[:, -self.args.pred_len:, :]).float() #生成一个全为0的张量
                #size = batch_size, label_len + pred_len, feature_dim
                ...
                #batch_x,frequency_map 没用到
                loss_value = criterion(batch_x, self.args.frequency_map, outputs, batch_y, batch_y_mark)
                # loss_sharpness = mse((outputs[:, 1:, :] - outputs[:, :-1, :]), (batch_y[:, 1:, :] - batch_y[:, :-1, :]))
                loss = loss_value  # + loss_sharpness * 1e-5
                train_loss.append(loss.item()) #计算损失
